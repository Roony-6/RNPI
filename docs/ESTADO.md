...
La refactorización de campos atómicos del personal (commit `45c5b7d`) está **propagada de forma consistente** en las cuatro capas: modelo ORM (`app/models/core.py`), schemas Pydantic (`app/schemas/personal.py`), router (`app/routers/personal.py`) y frontend (`static/js/app.js` → `nombreCompleto()`).
...
6. **Endurecimiento**: constraint UNIQUE en `folio_nna`, validación de confirmación de contraseña, exigir `SECRET_KEY` por entorno (fallar al arrancar si falta en producción).
